...
public class SimpleStatistics extends Statistics implements SteadyStateStatisticsForm //, ec.eval.ProvidesBestSoFar
    ...
    public void postEvaluationStatistics(final EvolutionState state)
        ...
        for(int x=0;x<state.population.subpops.length;x++)
            ...
            if (doGeneration) state.output.println(("Valores"+ec.Pacman.PacManProblem.ToDecimalArray((best_i[x]).genotypeToStringForHumans())), statisticslog);
    ...
    public void finalStatistics(final EvolutionState state, final int result)
        ...
        for(int x=0;x<state.population.subpops.length;x++ )
            ...
            if (doFinal) state.output.println(("Valores"+ec.Pacman.PacManProblem.ToDecimalArray((best_of_run[x]).genotypeToStringForHumans())), statisticslog);
